...
import { LiveDraftSelectionMessage, LiveDraftSelectionMessageType, liveDraftSelectionMiddleware } from './live-draft-selection.middleware'
import { LiveSessionService } from "../live-session";
import { DraftSelectionService } from "./draft-selection.service";
import { Logger } from "../logger";
import { v4 as uuid } from 'uuid';
import { generateRandomNumber } from "../random";
import { generateMockDraftSelection, generateMockDraftSelections } from "./draft-selection.mock";
...
describe('live-draft-selection-middleware', () => {
  ...
  it("processes a finalize draft selection message for a valid ticket", (done) => {
    ...
    when(draftSelectionService.finalizeOneForUser(matchers.anything(), matchers.anything(), matchers.anything())).thenResolve(generateMockDraftSelection())
    const expectedDraftSelections = generateMockDraftSelections()
    when(draftSelectionService.getAllForDraft(id)).thenResolve(expectedDraftSelections)
    ws = new WebSocket(urlToUse);
    ws.on("open", () => {
