...
it('displays a login error message if the login submission did not work', async () => {
  ...
  when(sessionService.createOne({ accessKey, password })).thenReject(new Error())
  render(<Login onSuccess={jest.fn()} sessionService={object<SessionService>()} />)
  await act(async () => {
  ...
  expect(errorMessage).toHaveTextContent(/The information submitted was incorrect./i)
})