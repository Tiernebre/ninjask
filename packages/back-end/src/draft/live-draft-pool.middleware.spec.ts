...
describe('LiveDraftPoolMiddleware', () => {
  ...
  it('advances to the next pokemon in the live draft pool when the NEXT message is received', (done) => {
    ...
    when(liveDraftPoolService.getLiveDraftPoolForOneWithId(id)).thenResolve(liveDraftPool)
    when(liveDraftPoolService.revealNextPokemonInLivePoolForId(id)).thenResolve(newLiveDraftPool)
    ws = new WebSocket(url)
  ...
  it('does nothing if an unsupported message is received', (done) => {
    ...
    ws.on('message', () => {
      verify(liveDraftPoolService.revealNextPokemonInLivePoolForId(id), { times: 0 })
      done()
    })
  })
})
