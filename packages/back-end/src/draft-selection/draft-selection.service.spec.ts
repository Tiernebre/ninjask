  ...
  describe("getAllForDraft", () => {
    ...
    it("properly maps a pokemon for each found draft selection", async () => {
      ...
      rowsFound.forEach((rowFound, index) => {
        when(pokemonService.getOneById(rowFound.pokemonId as number)).thenResolve(expectedPokemon[index])
      })
      const draftId = generateRandomNumber()
      when(draftSelectionRepository.getAllForDraftId(draftId)).thenResolve(rowsFound)
      const gottenSelections = await draftSelectionService.getAllForDraft(draftId)
