...
describe("LiveSessionService", () => {
  ...
  describe("createOne", () => {
    it.each(INVALID_NUMBER_CASES)(
      "throws ZodError if the SessionPayload has user id = %p",
      async (userId) => {
        await expect(
          liveSessionService.createOne({
    ...
    it("returns the created live session ticket", async () => {
      ...
      const createdLiveSessionTicketEntity =
        generateMockLiveSessionTicketEntity();
      when(
        liveSessionTicketRepository.create({ userId: session.userId })
      ).thenReturn(createdLiveSessionTicketEntity);
      when(
        liveSessionTicketRepository.save(createdLiveSessionTicketEntity)
      ).thenResolve(createdLiveSessionTicketEntity);
      await expect(liveSessionService.createOne(session)).resolves.toEqual({
        ticket: createdLiveSessionTicketEntity.token,
      });
    });
  });

  describe("redeemOne", () => {
    it.each(INVALID_STRING_CASES)(
      "throws ZodError if the provided ticket is %p",
      async (ticket) => {
        await expect(
          liveSessionService.redeemOne(ticket as string)
        ).rejects.toThrowError(ZodError);
      }
    );
    ...
    it("throws UnauthorizedError if a ticket was not found with the credentials provided", async () => {
      const ticket = uuid();
      when(
        liveSessionTicketRepository.findOne({ token: ticket, redeemed: false })
      ).thenResolve(undefined);
      await expect(liveSessionService.redeemOne(ticket)).rejects.toThrowError(
        UnauthorizedError
      );
    });
    ...
    it("returns the redeemed live session payload", async () => {
      ...
      when(
        liveSessionTicketRepository.findOne({ token: ticket, redeemed: false })
      ).thenResolve(expectedLiveSessionTicket);
      await expect(liveSessionService.redeemOne(ticket)).resolves.toEqual({
        userId: expectedLiveSessionTicket.userId,
      });
    });
  });
});
