...
const queryHourErrorMessage = () => screen.queryByRole('alert', { name: /Hour/i })
const getMinutesErrorMessage = () => screen.getByRole('alert', { name: /Minutes/i })
const queryMinutesErrorMessage = () => screen.queryByRole('alert', { name: /Minutes/i })
...
it.each([-1000, -1, 100, Number.MAX_SAFE_INTEGER])("displays an error message that hour needs to be within a range when %p is provided as the hour", async (hourToInput: number) => {
  const onSubmit = jest.fn()
  await act(async () => {
    render(<ChallengeResultForm onSubmit={onSubmit} />)
...
it.each([0, 1, 98, 99])("does not display an error message about the hour if %p is provided", async (hourToInput: number) => {
  const onSubmit = jest.fn()
  await act(async () => {
    render(<ChallengeResultForm onSubmit={onSubmit} />)
    user.type(getHourInput(), hourToInput.toString())
...
it.each([-1000, -1, 60, Number.MAX_SAFE_INTEGER])("displays an error message that minutes needs to be within a range when %p is provided", async (minutesToInput: number) => {
  const onSubmit = jest.fn()
  await act(async () => {
    render(<ChallengeResultForm onSubmit={onSubmit} />)
  ...
  const minutesErrorMessage = getMinutesErrorMessage()
  expect(minutesErrorMessage).toBeInTheDocument()
  expect(minutesErrorMessage).toHaveTextContent("Minutes must be between 0-59.")
  expect(getHourInput()).toBeInvalid()
  expect(onSubmit).not.toHaveBeenCalled()
})

it.each([0, 1, 58, 59])("does not display an error message about the minutes if %p is provided", async (minutesToInput: number) => {
  const onSubmit = jest.fn()
  await act(async () => {
    render(<ChallengeResultForm onSubmit={onSubmit} />)
    user.type(getMinutesInput(), minutesToInput.toString())
